        ...
        entityHealth =
            (HealthComponent)
                entity.getComponent(HealthComponent.class)
                    .orElseThrow(
                        () -> new MissingComponentException("HealthComponent HealSkill")
                    );
        if (
            entityHealth.getCurrentHealthpoints() !=
                entityHealth.getMaximalHealthpoints()) {
            int hp = entityHealth.getCurrentHealthpoints();
            setHealPoint(hp+getAmount());
            healLogger.info(
                getAmount() + "HP wurden regeneriert! Jetzt sind ("+(hp+getAmount())+"hp)");
            isFull = false;
        }else {
            isFull = true;
