...
public class TransformSkill implements ISkillFunction {
    ...
    private final static Logger transformLogger = Logger.getLogger(TransformSkill.class.getName());

    /**
     * Konstruktor
     * initialisiert die Variablen und ruft die Methode setupAnimation auf
    ...
    /**
     * Hier wird überprüft, welche Level der Spieler hat und die
     * passende Verwandlung angezeigt.
     */
    ...
    /**
     * Prüft bei jedes Frame, ob die Zeit abgelaufen ist und die
     * Original Textur laden soll oder nicht.
     */
    public void update(){
        if (requiredLevel){
            ...
            if (timer != 0 && !isActive){
                //transformLogger.info("Transformation endet in " + timer + "s : " + randomTimer + "s");
            }
        }
    }
    ...
    public void execute(Entity entity) {
        ...
        if (requiredLevel){ // Erforderliche Level erreicht
            if (isActive && isAnimationLoaded()){ // Beim ersten verwandlung isAction true
                ...
                if ( monster) {
                    if (hero.getMc().getCurrentManaPoint() >= 0){
                        hero.getMc().setCurrentManaPoint(hero.getMc().getCurrentManaPoint() - (int)mana);
                        transformLogger.info("Transform kostet ("+ mana
                            +") und Hero hat " + hero.getMc().getCurrentManaPoint() + " mana.");
                    }
                    if (mana <= 0 && hero.getHp().getCurrentHealthpoints() >= 0){
                        hero.getHp().setCurrentHealthpoints(hero.getHp().getCurrentHealthpoints() - (int) mana);
                        transformLogger.info("Mana sind alle, ziehe von Lebenspunkte ab ("+mana+" mana): " +
                            "Hero hat "
                    ...
                    if (hero.getHp().getCurrentHealthpoints() >= 0){
                        if (hero.getMc().getCurrentManaPoint() - mana <= 0){
                            ...
                            if (heroMana <= 0){
                                ...
                                System.out.println("Hero Mana ist unter 0 und " +
                                    "falls die Mana im negativen Bereich sind, Ziehe von Hero hp" +
                                    " ab!");
                                System.out.println("Es werden " + Math.abs(heroMana) + " mana von Hero hp abgezogen.");
                            }

                        }else{
                            hero.getMc().setCurrentManaPoint(hero.getMc().getCurrentManaPoint() - (int)mana);
                            System.out.println("Hero mana sind jetzt: " + hero.getMc().getCurrentManaPoint()
                                + ", davon sind " + mana + " abgezogen!!");
                            hero.getHp().setCurrentHealthpoints(hero.getHp().getCurrentHealthpoints() - (int) mana);
                            System.out.println("Zusaetzlich werden " + mana + "hp" +
                                " von HeroHP abgezogen! Hero hat noch " + hero.getHp().getCurrentHealthpoints()
                                + "hp");
                        }
                        /*
            ...
            transformLogger.info("Dein Level ist zu niedrig!!! ("+getClass().getSimpleName()+")");
        }
    }
    ...
    /**
     * @return true die Texturen sind geladen ansonsten false
     */
    private boolean isAnimationLoaded(){
        return pathToIdleLeft != null && pathToIdleRight != null
            && pathToRunLeft != null && pathToRunRight != null;
    }
