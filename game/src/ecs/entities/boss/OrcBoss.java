...
public class OrcBoss extends Boss {
    ...
    public String information() {
        return getClass().getSimpleName() + " hat folgende Skills und Eigenschaften: "
            + "\nSkills: " + meditationSkill.getClass().getSimpleName() +
            "(Shield wird "+ meditationSkill.shield() + "hp sein!)\n"
    ...
    private void attacke(){
        if (!skill1.isOnCoolDown()){
            bossLogger.info("Skill: " + meditationSkill.getClass().getSimpleName()
                + " wurde aktiviert!");
            skill1.execute(this);
    ...
    public void update(Set<Entity> entities, int level) {
        ...
        if (isCollision){
            ...
            if (time == new Random().nextInt(2)+1){
                ...
                bossLogger.info("heroAttack war erfolgreich!! " +
                    getClass().getSimpleName() + " hat (" + hero1.damage() + "damage's) bekommen!!\n" +
                    getClass().getSimpleName() + " hat noch " + hp.getCurrentHealthpoints() + "hp");
                timerStart = System.currentTimeMillis();
            }
        }
        if (!isCollision){
        ...
        if (hp.getCurrentHealthpoints() <= 0){
            heroXP = (int)xp;
            bossLogger.info(getClass().getSimpleName() + " wurde besiegt!\n" +
