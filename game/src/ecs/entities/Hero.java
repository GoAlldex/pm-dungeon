...
public class Hero extends Entity {
    ...
    /**
     * Blitzschlag initialisieren.
     * Wo der Maus point sich gerade befindet, wird der Blitz/Skill aktiviert.
     * Der CoolDown ist von der LightningLineSkill Klasse vor definiertes cool down
     * und der Skill hat ebenso sein eigene Timer für die Wiederaktivierung.
     */
    ...
    public void execute(Skill skill){
        ...
        if (thirdSkill != skill && fourthSkill != skill)
            skill.execute(this);
    }
    ...
    /**
     * Zeit für Blitzschlag.
     * ist die Zeit für Blitz breakTime erreicht,
     * setzt sich der Skill zurück und kann wieder abgefeuert werden.
     * Der breakTime ist random!
     */
    @Override
    public void update() {
        ...
        if (secondSkill.isOnCoolDown()){
            ...
            //heroLogger.info("CoolDown: "+lightningCoolDown+" | Timer: " + (timerForLightningEnd - timerForLightningStart) / (60*60) + "s");
        }
    ...
    public void setLootXP(long lootXP){
        xp.addXP(lootXP);
        heroLogger.info(
            "CurrentXP (Hero): " + xp.getCurrentXP() +
                "\nMaximale Xp (Hero): " + xp.getMaxXP()
        );
    ...
    /**
     * Die Hero HP werden nach der Kampf gesetzt.
     * Außerdem wird es überprüft, ob die hp <= 0 sind oder nicht
     * falls ja, wird die methode gameOver aufgerufen.
    ...
    public void setHp(int heroDMG){
        ...
        if (h <= 0){
            ...
            System.out.println("Hero hat nach dem Kampf: " +
                this.hp.getCurrentHealthpoints() + "hp");
            this.hp.setCurrentHealthpoints(
                this.hp.getCurrentHealthpoints() + reg
            );
    ...
    /**
     * @return nur für LightningLine, wird überprüft
     *          ob der Level zu niedrig ist oder der Skill wurde
     *          schon ausgeführt.
     */
     ...
     */
    public void info(){
        System.out.println("HP: " + hp.getCurrentHealthpoints() + " von "
        + hp.getMaximalHealthpoints());
        if (xp.getCurrentXP() <= 0){
            xp.setCurrentXP(0);
        }
        System.out.println("XP: " + xp.getCurrentXP() + " von " +
            xp.getMaxXP());
        System.out.println("CurrentLevel: " + xp.getCurrentLevel());
        if (mc.getCurrentManaPoint() <= 0)
            mc.setCurrentManaPoint(0);
        System.out.println("Mana: " + mc.getCurrentManaPoint() + " von " +
            mc.getMaxManaPoint());
    }
